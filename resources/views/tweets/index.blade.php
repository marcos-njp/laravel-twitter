        ...
        <div class="col-lg-7">
            ...
            <div class="modern-card p-4 border-0 mb-4 position-relative overflow-hidden" 
                ...
                <div class="d-flex align-items-start gap-3 position-relative z-1">
                    ...
                    <div>
                            <h4 class="mb-2 fw-bold text-white">Welcome to Birdie.</h4>
                        <p class="mb-0 text-white opacity-90" style="font-size: 0.95rem; line-height: 1.6; max-width: 90%;">
                            A minimalist space designed for curious minds. Share your daily learnings, research notes, and quick insights.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="modern-card p-4">
                ...
                <form action="{{ route('tweets.store') }}" method="POST" x-data="{ content: '' }">
                    ...
                    <div class="position-relative">
                        <textarea name="content" x-model="content" class="form-control border bg-light" rows="3" 
                            placeholder="What did you learn today?" style="resize: none; font-size: 1.1rem;" required maxlength="280"></textarea>
                        
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <small class="fw-bold transition-all" :class="content.length > 250 ? 'text-danger' : 'text-muted'" style="font-size: 0.85rem;">
                            ...
                            <button type="submit" class="btn btn-primary px-4 py-2 rounded-pill shadow-sm d-flex align-items-center gap-2" 
                                ...
                                <x-heroicon-s-paper-airplane style="width: 16px;" /> Post
                            </button>
                        </div>
                    </div>
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 px-2">
                    <h5 class="fw-bold mb-0 text-dark">Insights Feed</h5>
                <div class="dropdown">
                    <button class="btn btn-white bg-white border shadow-sm px-3 py-1 text-muted dropdown-toggle d-flex align-items-center gap-2 small" type="button" data-bs-toggle="dropdown">
                    ...
                    <div class="d-flex gap-3">
                        <a href="{{ route('users.show', $tweet->user) }}" class="text-decoration-none">
                            @if($tweet->user->avatar)
                                <img src="{{ asset('storage/' . $tweet->user->avatar) }}" alt="User Avatar" 
                                     class="rounded-1 object-fit-cover shadow-sm border" style="width: 48px; height: 48px;">
                            @else
                        ...
                        <div class="w-100">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div class="d-flex flex-column">
                                    ...
                                    <div class="d-flex align-items-center gap-2 text-muted small" style="font-size: 0.8rem;">
                                        ...
                                        @if($tweet->is_edited)
                                            <span>·</span>
                                            <span class="d-flex align-items-center gap-1 fst-italic text-warning-emphasis">
                            ...
                            <p class="mb-3 text-dark fs-5" style="line-height: 1.6;">{{ $tweet->content }}</p>
                            ...
                            <div class="d-flex align-items-center gap-4 border-top pt-3" style="border-color: #f5f5f4 !important;">
                                ...
                                <button onclick="toggleLike(this, {{ $tweet->id }})" class="btn p-0 text-decoration-none d-flex align-items-center gap-2 like-btn {{ $userHasLiked ? 'text-danger' : 'text-muted' }}" style="border: none; background: none;">
                                    <div class="icon-outline {{ $userHasLiked ? 'd-none' : '' }}"><x-heroicon-o-heart style="width: 20px;" /></div>
                                    <div class="icon-solid {{ $userHasLiked ? '' : 'd-none' }}"><x-heroicon-s-heart style="width: 20px;" /></div>
                                ...
                                <a href="{{ route('users.show', $user) }}" class="text-decoration-none">
                                    @if($user->avatar)
                                        <img src="{{ asset('storage/' . $user->avatar) }}" alt="Avatar" 
                                             class="rounded-circle object-fit-cover border" style="width: 40px; height: 40px;">
                                    @else
